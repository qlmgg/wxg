...
class MessageController extends SimpleController
{
    ...
    public function query($query,$user):Builder
    {
        ...
        $model = $model->where("to_user_id","=",$user->id);
        return $model;
    }
    ...
    public function show($id)
    {
        //查看详情时设置为已读
        $user = Auth::user();
        $m = $this->getModel()->with([])->where("to_user_id","=",$user->id)->findOrFail($id);
    ...
    {
        $user = $request->user();
        $message = Message::with([])->where("to_user_id","=",$user->id)->findOrFail($id);
        if($message){
