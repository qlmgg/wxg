...
class SiteConditionsController extends SimpleController
{
    ...
    public function query($query,$user):Builder
    {
        ...
        if(!$month_check_worker) throw new NoticeException("非法参数");
        $check_order = CheckOrder::with([])->find($month_check_worker->check_order_id);
        if($check_order->type==1){
            //共享查看 就是月检下的所有材料
            $model = $model->with([])->where("type","=",1)->where("month_check_id","=",$month_check_worker->month_check_id);
        }else{
            $model = $model->with([])->where("type","=",1)->where("worker_id","=",$user->id)->where("month_check_worker_id",$month_check_worker_id);
        }
    ...
    public function store(Request $request)
    {
        $data = $this->validate($request,[
            ...
            "siteConditions.remarks"=>["required","string","max:255"],
        ...
        return DB::transaction(function ()use($data,$user){
            ...
            //通过 month_check_id
            $c = $model->where("type","=",1)->where("month_check_id","=",$month_check_worker->month_check_id)->first();
            if($c){
                $c->update($conditions);
            }else{
